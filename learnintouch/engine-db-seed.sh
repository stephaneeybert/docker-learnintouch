...
sed -e "s/DB_PASSWORD/$LEARNINTOUCH_DB_PASSWORD/g" < /usr/bin/learnintouch/db_engine-db.sql > /usr/bin/learnintouch/db_engine-db-resolved.sql

/usr/bin/mariadb/install/bin/mysql --protocol=tcp -h mysql -P 3306 -u root -v < /usr/bin/learnintouch/db_engine-db-resolved.sql
